...
{
    public partial class ClaimDetailesReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {                
                ...
                sql = "SELECT NUM,POL_NO, CLAIM_NO,INSURED_NAME ,PAYEE_NAME, CLAIM_TYPE, BANK_NAME,BANK_BRANCH_NAME,ACC_NO,DATE_OF_CLAIM,SBU,nvl(Status,'Claim Entered') status , CHQNO, CHQDATE, Payment_type,Relashionship,claimant_name,Amount, Nic, EPF, REF from ( " +
                    ...
                    "CHQNO,to_char(CHQDATE,'YYYY-MM-DD') CHQDATE,DECODE(Payment_type, 'N', 'Normal', 'E', 'Ex-gratia') Payment_type,Relashionship,claimant_name,Amount, A.NIC , A.EPF, CONCAT(CONCAT(CONCAT(CONCAT(CONCAT('EPF', A.EPF ), 'SL'), SUBSTR(POL_NO, 6)), 'W'), CHQNO) as REF " +
                    ...
                    " Relashionship,claimant_name,Amount, A.NIC , A.EPF, null as REF From SLIC_CHP.VOU_DETAILS_MAS A  Join slic_chp.group_Master C on A. NIC = C.NIC " +
            ...
            else if (!hdfNIC.Value.Equals(""))
            {
                ...
                sql = "SELECT NUM,POL_NO, CLAIM_NO,INSURED_NAME ,PAYEE_NAME, CLAIM_TYPE, BANK_NAME,BANK_BRANCH_NAME,ACC_NO,DATE_OF_CLAIM,SBU,nvl(Status,'Claim Entered') status , CHQNO, CHQDATE, Payment_type,Relashionship,claimant_name,Amount, Nic, EPF, REF from ( " +
                    ...
                    "Status,CHQNO,to_char(CHQDATE,'YYYY-MM-DD') CHQDATE,DECODE(Payment_type, 'N', 'Normal', 'E', 'Ex-gratia') Payment_type,Relashionship,claimant_name,Amount, A.NIC , A.EPF, CONCAT(CONCAT(CONCAT(CONCAT(CONCAT('EPF', A.EPF ), 'SL'), SUBSTR(POL_NO, 6)), 'W'), CHQNO) as REF " +
        ...
        protected void GridView2_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            if (e.Row.RowIndex > -1)
            {
                ...
                if(e.Row.Cells[19].Text.Equals("") || e.Row.Cells[19].Text.Equals("&nbsp;"))
                {
                    e.Row.Cells[20].Text = "";
                }
